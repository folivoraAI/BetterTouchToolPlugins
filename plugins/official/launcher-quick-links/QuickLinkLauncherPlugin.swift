...
// BTT-Plugin-Identifier: com.folivora.btt.launcher.quicklinks
...
final class QuickLinkLauncherPlugin: NSObject, BTTLauncherPluginInterface {
    ...
    static let pluginIdentifier = "com.folivora.btt.launcher.quicklinks"
    ...
    func launcherResult(
        ...
        result.surfaceIdentifier = IDs.editorSurface
        result.launcherGroup = configuration.launcherGroup
        result.launcherDisplayMode = NSNumber(value: configuration.displayMode)
        result.commands = [
            command(
                id: Actions.edit,
                title: "Edit Quick Link",
                subtitle: "Change the name, URL template, browser, icon, display mode, or group.",
...
private struct QuickLinkConfiguration {
    ...
    static let launcherGroupKey = "launcherGroup"
    ...
    init(
        ...
        self.launcherGroup = Self.normalized(launcherGroup)
    }
    ...
    init(instance: BTTLauncherPluginInstance) {
        ...
        self.init(
            ...
            launcherGroup: Self.normalized(configuration[Self.launcherGroupKey] as? String)
                ?? Self.normalized(instance.launcherGroup)
        )
    }
    ...
    func instance(existingIdentifier: String?) -> BTTLauncherPluginInstance {
        ...
        instance.launcherGroup = launcherGroup
        ...
        instance.configuration = [
            ...
            Self.launcherGroupKey: launcherGroup ?? ""
        ]
        return instance
    }
...
private enum QuickLinkEditorSurfaceSize {
    static let widthKey  = "com.folivora.btt.launcher.quicklinks.editorWidth"
    static let heightKey = "com.folivora.btt.launcher.quicklinks.editorHeight"
...
private final class QuickLinkEditorSurface: NSObject, BTTLauncherPluginSurfaceInterface {
    ...
    func makeLauncherSurfaceView() -> NSView {
        var draft = QuickLinkEditorDraft(
        ...
        let groupChoices = QuickLinkGroupChoice.choices(
            from: delegate?.launcherGroups(launcherID: context.launcherID) ?? [],
            selectedGroup: draft.launcherGroupID
        )
        draft.launcherGroupID = QuickLinkGroupChoice.selectionID(
            for: draft.launcherGroupID,
...
private struct QuickLinkGroupChoice: Identifiable, Hashable {
    let id: String
    let title: String
    ...
    static func choices(
    ...
    ) -> [QuickLinkGroupChoice] {
        var choices = [
            QuickLinkGroupChoice(id: "", title: "No Group", systemImageName: "rectangle.stack")
        ]
        var seen = Set([""])

        for dictionary in dictionaries {
            guard let id = normalized(dictionary["id"] as? String) else { continue }
            let title = normalized(dictionary["title"] as? String) ?? id
            let systemImageName = normalized(dictionary["systemImageName"] as? String) ?? "folder"
            guard seen.insert(id.lowercased()).inserted else { continue }
            choices.append(QuickLinkGroupChoice(
                id: id,
                title: title,
        ...
        if let selectedGroup = normalized(selectedGroup),
           selectionID(for: selectedGroup, choices: choices) == selectedGroup {
            choices.append(QuickLinkGroupChoice(
                id: selectedGroup,
                title: selectedGroup,
    ...
    static func selectionID(
        for rawSelection: String?,
        choices: [QuickLinkGroupChoice]
    ) -> String {
        guard let rawSelection = normalized(rawSelection) else { return "" }
        if let exactID = choices.first(where: {
            $0.id.compare(rawSelection, options: [.caseInsensitive, .diacriticInsensitive]) == .orderedSame
        }) {
            return exactID.id
        }
        if let titleMatch = choices.first(where: {
            $0.title.compare(rawSelection, options: [.caseInsensitive, .diacriticInsensitive]) == .orderedSame
        }) {
...
private struct QuickLinkEditorDraft {
    ...
    init(configuration: QuickLinkConfiguration) {
        ...
        launcherGroupID = configuration.launcherGroup ?? ""
    }
    ...
    func configuration(browserChoices: [QuickLinkBrowserChoice]) -> QuickLinkConfiguration? {
        ...
        let normalizedLauncherGroup = trimmed(launcherGroupID)
        ...
        return QuickLinkConfiguration(
            ...
            launcherGroup: normalizedLauncherGroup.isEmpty ? nil : normalizedLauncherGroup
        )
    }
...
private struct QuickLinkEditorView: View {
    ...
    let groupChoices: [QuickLinkGroupChoice]
    ...
    init(
        ...
        groupChoices: [QuickLinkGroupChoice],
        ...
        VStack(spacing: 0) {
            ScrollView {
                VStack(alignment: .leading, spacing: 22) {
                    ...
                    formRow("Group") {
                        Picker("", selection: $draft.launcherGroupID) {
                            ForEach(groupChoices) { group in
                                Label(group.title, systemImage: group.systemImageName)
                                    .tag(group.id)
                            }
                        }
                        .labelsHidden()
